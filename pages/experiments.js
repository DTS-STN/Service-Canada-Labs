...
export default function experiments(props) {
  ...
  const [filteredExperiements, setfilteredExperiements] = useState(props.experimentData);
  ...
  const handleFilter = (event) => {
    if(parseInt(event.target.id) === 0){
      ...
      setfilteredExperiements(props.experimentData.filter(experiment => experiment.tag.id === parseInt(event.target.id)));
    }
  }
