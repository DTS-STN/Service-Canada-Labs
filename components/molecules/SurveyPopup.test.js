...
jest.mock('next-i18next', () => ({
  useTranslation: () => ({
    t: (key) => ({
      ...
      'surveyPopup.titleExpanded': 'Tell us what you think of Service Canada Labs',
      'surveyPopup.titleCollapsed': 'Tell us what you think',
      'surveyPopup.description': 'You can help shape this website too. Take a few minutes to tell us about your experience.',
...
describe('SurveyPopup', () => {
  it('renders in initial collapsed state', () => {
    ...
    expect(screen.queryByText('Tell us what you think of Service Canada Labs')).not.toBeInTheDocument();
  });

  it('expands when clicked and shows full content', () => {
    render(<SurveyPopup />);
    
    const toggleButton = screen.getByRole('button', { name: /Open feedback survey/i });
    fireEvent.click(toggleButton);

    expect(screen.getByText('Tell us what you think of Service Canada Labs')).toBeInTheDocument();
    expect(screen.getByText('You can help shape this website too. Take a few minutes to tell us about your experience.')).toBeInTheDocument();
  ...
  it('collapses when "No thanks" is clicked', () => {
    ...
    // Open the popup
    fireEvent.click(screen.getByRole('button', { name: /Open feedback survey/i }));
    // Click "No thanks"
  ...
  it('moves focus to "No thanks" button when opened', () => {
    ...
    expect(document.activeElement).toBe(screen.getByRole('button', { name: /No thanks/i }));
  });
  ...
  describe('text overrides', () => {
    ...
    it('uses custom text when provided', () => {
      ...
      expect(screen.getByRole('complementary')).toHaveAttribute('aria-label', customText.ariaLabel);
      
      // Open popup
      fireEvent.click(screen.getByRole('button', { name: customText.openButtonLabel }));
      ...
      expect(screen.getByRole('button', { name: customText.noThanksText })).toBeInTheDocument();
      expect(screen.getByRole('button', { name: customText.takeSurveyText })).toBeInTheDocument();
      expect(screen.getByRole('group')).toHaveAttribute('aria-label', customText.optionsGroupLabel);
    });
    ...
    it('falls back to translations when props are not provided', () => {
      ...
      expect(screen.getByText('Tell us what you think')).toBeInTheDocument();
      expect(screen.getByRole('complementary')).toHaveAttribute('aria-label', 'Website feedback survey');
      
      fireEvent.click(screen.getByRole('button', { name: /Open feedback survey/i }));
      
      expect(screen.getByText('Tell us what you think of Service Canada Labs')).toBeInTheDocument();
      expect(screen.getByRole('button', { name: 'No thanks' })).toBeInTheDocument();
      expect(screen.getByRole('button', { name: 'Take the survey' })).toBeInTheDocument();
    });
